...
void handleErrorCode(ErrorCode code, ErrorInfo *errorInfo)
{
    ...
    case EOF_RECORD_ERROR:
        snprintf(errorInfo->message, sizeof(errorInfo->message), "EOF record not found");
        break;
    default:
...
void callFunction(ErrorCode code, const char *message)
{
    printf("Error Code: %d\n", code);
...
int main(void)
{
    ...
    if (result != 0)
    {
        callFunction(errorInfo.code, errorInfo.message);
    }
